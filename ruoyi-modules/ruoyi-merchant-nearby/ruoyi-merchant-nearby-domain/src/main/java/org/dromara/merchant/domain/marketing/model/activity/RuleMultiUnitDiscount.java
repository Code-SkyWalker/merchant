...
public class RuleMultiUnitDiscount implements Rule {
    ...
    public BigDecimal calculateTotalDiscount(BigDecimal totalPrice, Integer quantity) {
        ...
        // 按满足条件的最高档次计算
        Condition bestCondition = this.content.equals(Content.AMOUNT) ? findBestAmountCondition(totalPrice) : findBestQuantityCondition(quantity);
        if (bestCondition == null) return BigDecimal.ZERO;

        // 根据优惠方式计算优惠金额
        if (Way.LOOP.equals(way)) {
            // 循环优惠："每"满amount减discount
            return calculateLoopDiscount(totalPrice, bestCondition);
        } else {
            // 阶梯优惠：按满足的最高条件计算
            return calculateTieredDiscount(totalPrice, bestCondition);
        }
    }
     ...
     * @return 循环优惠金额
     */
    private BigDecimal calculateLoopDiscount(BigDecimal eligibleTotal, @NonNull Condition bestCondition) {
     ...
     * @return 阶梯优惠金额
     */
    private BigDecimal calculateTieredDiscount(BigDecimal eligibleTotal, @NonNull Condition bestCondition) {
        return getDiscountAmount(bestCondition, eligibleTotal);
    }
     ...
     * 为梯次优惠找到最佳条件
     *
     * @param eligiblePriceTotal 符合条件商品的总金额
     * @return 最佳条件
     */
    private Condition findBestAmountCondition(BigDecimal eligiblePriceTotal) {
        return discounts.stream()
            .filter(condition -> eligiblePriceTotal.compareTo(condition.getAmount()) >= 0)
     ...
     * 为循环优惠找到最佳条件
     *
     * @param eligibleQuantityTotal 符合条件商品的总件数
     * @return 最佳条件
     */
    private Condition findBestQuantityCondition(Integer eligibleQuantityTotal) {
        return discounts.stream()
                .filter(condition -> condition.getAmount().compareTo(new BigDecimal(eligibleQuantityTotal)) < 0)
                .max(Comparator.comparing(Condition::getAmount))// 选择满足条件的最大金额条件
                .orElse(null);
    }
    ...
    private BigDecimal getDiscountAmount(Condition condition, BigDecimal eligibleTotal) {
        ...
        if (Requirement.PRICE.equals(requirement)) {
            ...
            BigDecimal discountRate = condition.getDiscount().divide(BigDecimal.TEN, 2, RoundingMode.HALF_UP);
